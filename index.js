...
async function applyImageEffect(imagePath) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      try {
        execSync(
          `ffmpeg -i ${imagePath} -vf eq=brightness=${randomBrightness}:saturation=${randomSaturation},scale=${newWidth}:${newHeight},noise=alls=${noise}:allf=t+u,boxblur=2:0.35 -map_metadata -1 ${outputImagePath}`
        );
